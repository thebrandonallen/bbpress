...
/**
 * BuddyPress Group Forums (bbPress 1.1) Converter
 * - Default WordPress Install with shared database tables
 * -  BuddyPress 1.6.x  _bp_*
 * -  bbPress 1.x       _bb_*
 *
 * @since bbPress (rXXXX)
 * @link Codex Docs https://codex.bbpress.org/import-forums/bbpress-1-x-buddypress-group-forums
 */
class BuddyPressLegacy extends BBP_Converter_Base {

	/**
	 * Main constructor
	 *
	 * @uses BuddyPress::setup_globals()
	 */
	...
	public function setup_globals() {
		...
		// Forum visibily
		$this->field_map[] = array(
			'from_tablename'  => 'bp_groups_groupmeta',
			'from_fieldname'  => 'group_id',
			'join_tablename'  => 'bb_forums',
			'join_type'       => 'LEFT',
			'join_expression' => 'ON bp_groups_groupmeta.meta_value = bb_forums.forum_id AND bp_groups_groupmeta.meta_key = "forum_id"',
			'to_type'         => 'forum',
			'to_fieldname'    => 'post_status',
			'callback_method' => 'callback_group_id'
		);

 		// Forum type (bbPress v1.x Forum > 0 or Category = 0, Stored in postmeta)
		$this->field_map[] = array(
			'from_tablename'  => 'bb_meta',
			'from_fieldname'  => 'meta_value',
			'join_tablename'  => 'bb_forums',
			'join_type'       => 'LEFT',
			'join_expression' => 'ON bb_meta.object_id = bb_forums.forum_id AND bb_meta.meta_key = "forum_is_category"',
			'to_type'         => 'forum',
		...
		$this->field_map[] = array(
			'from_tablename'  => 'bb_posts',
			'from_fieldname'  => 'post_text',
			'join_tablename'  => 'bb_topics',
			'join_type'       => 'INNER',
			'join_expression' => 'USING (topic_id) WHERE bb_posts.post_position IN (0,1)',
			'to_type'         => 'topic',
			'to_fieldname'    => 'post_content',
		...
		// Topic id.
		$this->field_map[] = array(
			'from_tablename'  => 'bb_term_relationships',
		...
		// Taxonomy ID.
		$this->field_map[] = array(
			'from_tablename'  => 'bb_term_taxonomy',
			'from_fieldname'  => 'term_taxonomy_id',
			'join_tablename'  => 'bb_term_relationships',
			'join_type'       => 'INNER',
		...
		// Term text.
		$this->field_map[] = array(
			'from_tablename'  => 'bb_terms',
			'from_fieldname'  => 'name',
			'join_tablename'  => 'bb_term_taxonomy',
			'join_type'       => 'INNER',
			'join_expression' => 'USING (term_id) WHERE bb_term_taxonomy.taxonomy = "bb_topic_tag"',
			'to_type'         => 'tags',
			'to_fieldname'    => 'name'
		);
		...
		$this->field_map[] = array(
			...
			'join_tablename'  => 'bb_term_taxonomy',
			'join_type'       => 'INNER',
			'join_expression' => 'USING (term_id) WHERE bb_term_taxonomy.taxonomy = "bb_subscribe"',
			'to_type'         => 'topic_subscriptions',
		...
		$this->field_map[] = array(
			'from_tablename'  => 'bb_topics',
			'from_fieldname'  => 'topic_id',
			'join_tablename'  => 'bb_posts',
			'join_type'       => 'INNER',
			'join_expression' => 'USING (topic_id) WHERE bb_posts.post_position NOT IN (0,1)',
			'to_type'         => 'reply',
	...
	/**
	 * Takes the passed group id and queries for the group status. From there, we
	 * conform it to bbPress standards.
	 *
	 * @since (rXXXX)
	 *
	 * @param string $field Group id.
	 * @return string Group forum status.
	 */
	protected function callback_group_id( $field ) {

		// Use the provided group id to get the group status
		$status = (string) $this->wpdb->get_var( $this->wpdb->prepare( 'SELECT status FROM ' . $this->wpdb->prefix . 'bp_groups WHERE id = %d LIMIT 1', $field ) );

		// Conform the BP status to our use
		switch ( $status ) {
			case 'private' :
			...
			case '' :
				$status = bbp_get_orphan_status_id();
				break;
